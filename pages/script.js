...
function addCards(data) {
    for (let i = 0; i < data.length; i++) {
        ...
        cardsTitle.textContent = data[i].name;
...
function formSubmitHandler(evt) { console.log('formSubmit 1111');
...
function formAddPlaceSubmitHandler(evt) { console.log('formSubmit 22222');
    evt.preventDefault(); // Эта строчка отменяет стандартную отправку формы.
    // Получаем значение полей jobInput и nameInput из свойства value
    ...
    // Выбераем элементы, куда должны быть вставлены значения полей
    // Вставляем новые значения с помощью textContent
    console.log('placeNameInputValue = ', placeNameInputValue);
    console.log('placeImgInputValue = ', placeImgInputValue);
...
console.log('formAddPlaceElement = ',formAddPlaceElement);
